...
def preprocess_text(text):
    ...
    text = "".join(
        [
            ch if ch in allowed_characters
            else replacement_character if ch in characters_to_replace
            else ch
            for ch in text
        ]
    )
...
def generate_html_ngram_matrix(n_grams, m_grams, counts):
    ...
    html_content += "th, td { border: 1px solid #ddd; padding: 8px; text-align: center; }"
...
generate_html_ngram_matrix(most_common_n_grams, most_common_m_grams, ngram_combinations_counts)
